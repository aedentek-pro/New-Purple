...
// Helper function to convert backend user (with _id) to frontend User (with id)
...
const App: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const loadUsers = async () => {
            try {
                ...
                db.saveUsers(mappedUsers);
            } catch (error) {
                console.error('Error loading users from backend, using local storage:', error);
        ...
        // Load other data from local storage (can be migrated to API later)
    ...
    const handleInitiateSignup = async (
        ...
        try {
            const { exists } = await api.auth.checkEmail(trimmedEmail);
            if (exists) {
                return false;
            }
            ...
            setPendingSignupData({ name, email: trimmedEmail, password, role });
    ...
    const handleVerifyOtpAndSignup = async (otp: string): Promise<boolean> => {
        if (otp === signupOtp && pendingSignupData) {
            try {
                ...
                const updatedUsers = [...users, mappedUser];
